...
def main():
    ...
    def _add_gateway_validate_args(parser):
        ...
        parser.add_argument(
            "--base-url",
            default=None,
            help="Gateway API base URL (default: API_SERVER_HOST/API_SERVER_PORT or http://127.0.0.1:8642)",
        )

    gateway_validate = gateway_subparsers.add_parser(
        "validate",
        help="Run read-only post-restart gateway validation checks",
        description="Validate gateway health, API auth, logs, memory readiness, and git state without restarting services.",
    )
