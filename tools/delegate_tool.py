        ...
        if n_tasks == 1:
            ...
            if child is None:
                ...
                result = _run_claude_code_task(
                    ...
                    parent_agent=parent_agent,
                    primary_model=str(
                        coding_cfg.get("primary_model") or "claude-opus-4-7"
                    ),
                    primary_effort=str(coding_cfg.get("primary_effort") or ""),
                    fallback_model=str(coding_cfg.get("fallback_model") or ""),
                    backup_runtime=backup_runtime,
                    max_iterations=effective_max_iter,
            ...
            with DaemonThreadPoolExecutor(max_workers=max_children) as executor:
                ...
                for i, t, child in children:
                    # Copy ContextVar state per-iteration so each child runs
                    # under an isolated context (upstream isolation pattern).
                    ...
                    if child is None:
                        ...
                        future = executor.submit(
                            ...
                            parent_agent=parent_agent,
                            primary_model=str(
                                coding_cfg.get("primary_model")
                                or "claude-opus-4-7"
                            ),
                            primary_effort=str(
                                coding_cfg.get("primary_effort") or ""
                            ),
                            fallback_model=str(
                                coding_cfg.get("fallback_model") or ""
                            ),
                            backup_runtime=backup_runtime,
                            max_iterations=effective_max_iter,
                            saved_parent_tool_names=_parent_tool_names,
